      ...
      <div className="relative z-10 mx-auto max-w-7xl  pb-18 sm:px-6 lg:px-8 lg:py-18">
        <div className="grid gap-8 lg:grid-cols-[0.92fr_1.08fr] lg:items-stretch">
            ...
            <div className="absolute inset-x-0 bottom-0 p-4 sm:p-6 px-2">
              <div className="mt-4 grid grid-cols-4 gap-0.5 sm:gap-2 px-1">
                {matchStats.map((item) => {
                  ...
                  return (
                    <div key={item.label} className="rounded-sm border-white/10 p-1.5 sm:p-2.5">
                      <p className="mt-1 sm:mt-2 text-sm sm:text-lg font-black text-white">{item.value}</p>
                      <p className="mt-0.5 sm:mt-1 text-[8px] sm:text-[10px] uppercase text-white/45">{item.label}</p>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="grid content-between gap-3 px-4">
            <div>
              <p className="text-[0.5rem] sm:text-xs font-medium uppercase tracking-[0.28em] text-slate-500">Built for competition</p>
              ...
              <p className="mt-3 max-w-2xl text-xs leading-7 text-slate-600 sm:text-base">
            ...
            <div className="grid gap-2 md:grid-cols-3">
              {featureCards.map((item) => {
                ...
                return (
                  <article key={item.title} className="group sm:rounded-lg sm:border border-black/10 bg-black/[0.02] p-1 sm:p-3.5 transition-all duration-300 hover:border-red-500/30 hover:bg-black/[0.04]">
                    <h3 className="mt-3 text-sm font-semibold text-slate-900">{item.title}</h3>
                    <p className="mt-2 text-xs sm:text-sm leading-6 text-slate-600">{item.copy}</p>
                  </article>
                );
              })}
            </div>
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                {stages.map((stage) => (
                  <div key={stage.label} className="border-l border-black/10 pl-3">
                    <p className="text-xl sm:text-2xl font-black text-red-500">{stage.value}</p>
                    <h3 className="mt-1 sm:mt-1.5 text-xs sm:text-sm font-semibold text-slate-900">{stage.label}</h3>
                    <p className="mt-1 sm:mt-1.5 text-[10px] sm:text-xs leading-4 sm:leading-5 text-slate-600">{stage.detail}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="hidden flex-row flex-nowrap gap-2 sm:flex">
              ...
              <Link href="/event-details" className="flex-1 sm:flex-none text-center rounded-lg border border-black/15 bg-black/5 px-4 py-2.5 sm:px-5 text-[13px] sm:text-sm font-semibold sm:tracking-[0.12em] text-slate-900 transition hover:bg-black/10">
                Events
              </Link>
            </div>
          </div>
